...
def capture_rpi(name):
    import picamera
    file = os.path.join(os.path.realpath(
        '/home/ubuntu'), '{}_qrcapture.png'.format(name))
...
def capture_webcam(name):
    file = os.path.join(os.path.realpath(
        '/home/ubuntu'), '{}_qrcapture.jpg'.format(name))
    cmd = ('gst-launch-1.0 v4l2src device=/dev/{} num-buffers=1 ! video/x-raw,width=640,height=480 ! jpegenc !'
...
def display_code(qr):
    with open('/dev/ttymxc0', 'wb+', buffering=0) as term:
